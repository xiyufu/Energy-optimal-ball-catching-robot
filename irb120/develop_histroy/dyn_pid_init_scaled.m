function [solver, scale, Sign] = dyn_pid_init(Ts)
...
q_nsi = SX.sym('q',6);
qd_nsi = SX.sym('qd',6);
qdd_nsi = SX.sym('qdd',6);
% scale = diag([1/50,1/50,1/10,50,1/2,1]);
scale = diag([1,1,1,100,100,100]);
q = scale*q_nsi;
qd = scale*qd_nsi;
qdd = scale*qdd_nsi;
...
x = vertcat(q_nsi,qd_nsi,nu);
% Algebraic states: qdotdot
z = qdd_nsi;
% Ordinary differential equations
rhs = vertcat(scale\qd,scale\qdd,scale\(Ki.*pid_e2));
% Algebraic equations
fz = pid_u - HbTh;
...
dae.alg = fz;
opts.tf = Ts;
opts.rootfinder = 'nlpsol';
opts.rootfinder_options = struct('nlpsol','ipopt');
solver = integrator('integrator','collocation',dae,opts);
